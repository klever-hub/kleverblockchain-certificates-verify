...
export default function CertificateStatus({ 
  ...
  const statusConfig = {
    ...
    'fields-verified': {
      ...
      description: 'All certificate details have been validated. Upload PDF for complete verification.'
    },
    'fully-verified': {
      color: 'text-accent',
      bgColor: 'bg-accent/20',
  ...
  return (
    <div className={`glass-card p-6 ${config.borderColor} border-2 animate-scale-in`}>
      <div className="flex items-start gap-4">
        <div className={`${config.bgColor} p-3 rounded-full ${config.color}`}>
          {config.icon}
        </div>
        <div className="flex-1">
          <h3 className={`text-xl font-bold mb-1 ${config.color}`}>
            {config.title}
          </h3>
              ...
              <div className="w-full bg-dark-lighter rounded-full h-2 overflow-hidden">
                <div 
                  className={`h-full transition-all duration-500 ${
                    status === 'fully-verified' ? 'bg-accent' : 
